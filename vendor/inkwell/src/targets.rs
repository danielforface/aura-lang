...
impl Target {
    ...
    pub fn initialize_native(config: &InitializationConfig) -> Result<(), String> {
        // NOTE (Aura/Windows): Some LLVM distributions (including the official Windows installer)
        // ship an LLVM C API that does not export the `LLVM_Initialize*` family.
        // In those environments, referencing these symbols causes link failures even if you never
        // attempt to emit machine code.
        //
        // Keep the real initialization behavior behind a feature so consumers can opt in when
        // they have a full LLVM build.
        #[cfg(feature = "llvm-target-init")]
        {
            use llvm_sys::target::{
        ...
        #[cfg(not(feature = "llvm-target-init"))]
        {
            let _ = config;
            Err("LLVM target initialization is disabled (inkwell feature `llvm-target-init` is off).".to_string())
        }
    }
    ...
    pub fn initialize_all(config: &InitializationConfig) {
        ...
        {
            let _ = config;
            // No-op when target initialization is disabled.
        }
    }
