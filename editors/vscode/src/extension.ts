...
type AuraProofsParams = { uri: string };
type AuraProofsResponse = { uri: string; diagnostics: vscode.Diagnostic[] };

let proofsPanel: vscode.WebviewPanel | undefined;
...
function formatSeverity(sev: vscode.DiagnosticSeverity | undefined): string {
  switch (sev) {
...
async function renderProofsPanel(docUri?: vscode.Uri) {
  if (!client) {
  ...
  const res = await client.sendRequest<AuraProofsResponse>("aura/proofs", {
    uri: uri.toString(),
  } satisfies AuraProofsParams);
  ...
  if (!proofsPanel) {
    proofsPanel = vscode.window.createWebviewPanel(
      "auraProofs",
      "Aura Proofs",
...
type ProofState = "idle" | "computing" | "verified" | "failed";
...
function parseNexusProofMeta(d: vscode.Diagnostic): NexusProofMeta | undefined {
  const code = d.code;
  ...
  const meta: NexusProofMeta = { plugin };
  for (const p of parts) {
    ...
    if (key === "mask") meta.mask = val;
    if (key === "range") meta.range = val;
  }
  return meta;
}

function findNexusDiagnosticAt(doc: vscode.TextDocument, pos: vscode.Position): vscode.Diagnostic | undefined {
  const ds = vscode.languages.getDiagnostics(doc.uri);
  for (const d of ds) {
    const meta = parseNexusProofMeta(d);
    if (!meta) continue;
    if (d.range.contains(pos)) return d;
  }
  return undefined;
}

function findAuraToml(startPath: string): string | undefined {
...
type PluginRow = { name: string; trusted?: boolean; source: "manifest" | "default" };

function parsePluginsFromAuraToml(raw: string): PluginRow[] {
...
function activePluginsForWorkspace(): PluginRow[] {
...
class NexusPluginProvider implements vscode.TreeDataProvider<NexusPluginItem> {
  private _onDidChangeTreeData = new vscode.EventEmitter<NexusPluginItem | null | undefined>();
  onDidChangeTreeData: vscode.Event<NexusPluginItem | null | undefined> = this._onDidChangeTreeData.event;

  refresh() {
    this._onDidChangeTreeData.fire(undefined);
  }

  getTreeItem(element: NexusPluginItem): vscode.TreeItem {
    return element;
  }

  getChildren(): Thenable<NexusPluginItem[]> {
    const rows = activePluginsForWorkspace();
    return Promise.resolve(rows.map((r) => new NexusPluginItem(r)));
  }
}

function pickCargoPath(config: vscode.WorkspaceConfiguration): string {
...
function envAuraHome(): string | undefined {
...
function workspaceBinaryCandidates(exeName: string): string[] {
...
function auraLspBinaryPath(workspaceRoot: string): string {
  const exe = process.platform === "win32" ? "aura-lsp.exe" : "aura-lsp";
  return path.join(workspaceRoot, "target", "debug", exe);
}

function auraLspBinaryPathRelease(workspaceRoot: string): string {
  const exe = process.platform === "win32" ? "aura-lsp.exe" : "aura-lsp";
  return path.join(workspaceRoot, "target", "release", exe);
}

function resolveAuraLspBinary(context: vscode.ExtensionContext, workspaceRoot: string): string {
  const exe = process.platform === "win32" ? "aura-lsp.exe" : "aura-lsp";
...
function resolveAuraBinary(context: vscode.ExtensionContext, config: vscode.WorkspaceConfiguration, workspaceRoot: string): string {
...
function auraInterpretBinaryPath(workspaceRoot: string, profile: "debug" | "release"): string {
  const exe = process.platform === "win32" ? "aura-interpret.exe" : "aura-interpret";
  return path.join(workspaceRoot, "target", profile, exe);
}

async function buildAuraInterpret(context: vscode.ExtensionContext, cargoPath: string, workspaceRoot: string) {
  await new Promise<void>((resolve, reject) => {
    ...
    child.on("exit", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`cargo build (aura-interpret) failed with code ${code}`));
    });
  });
}

function replHtml(): string {
  return `<!doctype html>
  <html>
    <head>
      <meta charset="utf-8" />
...
async function buildAuraLsp(context: vscode.ExtensionContext, cargoPath: string, workspaceRoot: string, status: vscode.StatusBarItem) {
...
function computeProofStateForActiveDoc(): { state: ProofState; errors: number; warnings: number } {
  const editor = vscode.window.activeTextEditor;
  ...
  const state: ProofState = errors > 0 ? "failed" : "verified";
  return { state, errors, warnings };
}

function updateStatusBar(status: vscode.StatusBarItem, state: ProofState) {
  if (state === "idle") {
...
function pickPrimaryDiagnosticsByLine(ds: readonly vscode.Diagnostic[]): { errors: vscode.Diagnostic[]; warnings: vscode.Diagnostic[] } {
  const errorsByLine = new Map<number, vscode.Diagnostic>();
  const warningsByLine = new Map<number, vscode.Diagnostic>();
...
function refreshDecorations(
  errorDeco: vscode.TextEditorDecorationType,
  warningDeco: vscode.TextEditorDecorationType,
  iotVerifiedDeco: vscode.TextEditorDecorationType,
  aiVerifiedDeco: vscode.TextEditorDecorationType,
) {
  const editor = vscode.window.activeTextEditor;
  if (!editor || editor.document.languageId !== "aura") {
    return;
  ...
  const verifiedIot: vscode.Diagnostic[] = [];
  const verifiedAi: vscode.Diagnostic[] = [];
  for (const d of diags) {
    const meta = parseNexusProofMeta(d);
    if (!meta || meta.kind !== "verified") continue;
    const p = meta.plugin.toLowerCase();
    if (p.includes("iot")) verifiedIot.push(d);
    else if (p.includes("ai")) verifiedAi.push(d);
  }

  const toOptions = (d: vscode.Diagnostic): vscode.DecorationOptions => ({
    range: d.range,
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  client = new LanguageClient("aura", "Aura Language Server", serverOptions, clientOptions);
  ...
  context.subscriptions.push(
    vscode.languages.registerHoverProvider({ language: "aura" }, {
      provideHover(doc, pos) {
        ...
        if (meta.mask) lines.push(`Bitmask: \`${meta.mask}\``);
  ...
  context.subscriptions.push(
    vscode.languages.registerInlayHintsProvider({ language: "aura" }, {
      provideInlayHints(doc: vscode.TextDocument, _range: vscode.Range, _token: vscode.CancellationToken) {
        const ds = vscode.languages.getDiagnostics(doc.uri);
        const hints: vscode.InlayHint[] = [];

        for (const d of ds) {
          const meta = parseNexusProofMeta(d);
          if (!meta || meta.kind !== "verified" || !meta.range) continue;

          // Prefer showing these only for IoT-ish proofs.
          if (!meta.plugin.toLowerCase().includes("iot")) continue;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("aura.openRepl", async () => {
      ...
      panel.webview.onDidReceiveMessage(async (msg) => {
        ...
        context.subscriptions.push(
          vscode.commands.registerCommand("aura.showProofs", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("aura.exportProofs", async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("aura.clearProofCache", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("aura.toggleProofDebug", () => {
      ...
      vscode.window.showInformationMessage(
        `Aura: debug output ${!current ? "enabled" : "disabled"}`
      );
    })
  );
